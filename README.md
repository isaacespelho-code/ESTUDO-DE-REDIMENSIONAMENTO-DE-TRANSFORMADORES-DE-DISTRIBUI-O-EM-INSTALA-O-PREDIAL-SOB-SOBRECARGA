#  ESTUDO DE REDIMENSIONAMENTO DE TRANSFORMADORES DE DISTRIBUIÇÃO EM INSTALAÇÃO PREDIAL SOB SOBRECARGA
Trabalho de Conclusão de Curso
Este repositório reúne o estudo técnico sobre os impactos da sobrecarga em transformadores de potência em um sistema de distribuição elétrica de um condomínio residencial composto por edifícios de 10 pavimentos, localizado no interior do Estado de São Paulo. A investigação foi motivada pelo subdimensionamento identificado no cálculo de demanda, que levou os transformadores existentes a operarem acima de sua capacidade nominal.
O trabalho analisa a coerência entre os cálculos de demanda das torres e as especificações dos transformadores instalados, utilizando normas nacionais, internacionais e diretrizes da concessionária para embasar a avaliação. Os resultados apontam que os transformadores originais operavam em sobrecarga crítica, sujeitos a superaquecimento e aceleração do processo de envelhecimento térmico do isolamento.
Com base nisso, o estudo propõe o redimensionamento dos transformadores, garantindo que a demanda prevista seja atendida com segurança, confiabilidade e melhor desempenho operacional. 

#  Objetivos Gerais do Trabalho
- Avaliar o desempenho do transformador em operação sob regime de sobrecarga;
- Determinar as causas e consequências técnicas da sobrecarga, considerando aspectos térmicos e operacionais;
- Redimensionar o sistema elétrico secundário e propor alternativas adequadas de transformação;
- Garantir conformidade com as normas técnicas vigentes e boas práticas de engenharia;

#  Conteúdo Técnico
O estudo aborda todas as etapas de avaliação e cálculo do sistema elétrico, incluindo:

#  Análises de Demanda e Sobrecarga
Levantamento das cargas instaladas e demandas por pavimento e áreas comuns;
Aplicação da ND-26 (concessionária) para cálculo de demanda por torre;
Identificação do regime de sobrecarga do transformador existente;

#  Proposta de Redimensionamento
Definição da nova potência nominal do transformador;
Análise técnica e justificativa para a escolha da nova capacidade transformadora;
Proposição de novos parâmetros operacionais para garantir a confiabilidade do sistema.

#  Instituição
- Centro Universitário Hermínio Ometto – Araras/SP
- Curso: Engenharia Elétrica
- Período de entrega: Junho de 2025

#  Restrições de Uso
Este trabalho tem finalidade exclusivamente acadêmica e didática.
A aplicação dos cálculos, dimensionamentos ou simulações em sistemas reais deve ser realizada somente por profissionais habilitados.
É proibido qualquer uso comercial parcial ou total sem autorização expressa do autor.
Para solicitações externas ou dúvidas sobre o uso do material, entre em contato.

#  Contato
- Autor: Isaac Zulin Memari Espelho
- Email: espelho@alunos.fho.edu.br
- Orientador: Prof. Dr. Daniel Augusto Pagi Ferreira
- Email: danielferreira@fho.edu.br
